...
describe("MCP stdio contract", () => {
  it("negotiates, lists primitives, validates a deck, and reads resources", async () => {
    const client = new Client({ name: "presentlab-mcp-test", version: "0.1.0" });
    ...
    try {
      ...
      expect(templates).toHaveLength(770);
      ...
      expect((themesTool.structuredContent as { themes: unknown[] }).themes).toHaveLength(770);
